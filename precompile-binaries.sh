...
curl --silent http://www.haskell.org/ghc/dist/$ghcversion/ghc-$ghcversion-x86_64-unknown-linux.tar.bz2|tar xj
cd ghc-$ghcversion/
...
rm $HOME/ghc/lib/ghc-$ghcversion/haddock
rm -r $HOME/ghc/lib/ghc-$ghcversion/html
rm -r $HOME/ghc/lib/ghc-$ghcversion/latex
# rm $HOME/ghc/bin/hp*
...
echo "" > $HOME/ghc/lib/ghc-$ghcversion/ghc-usage.txt
echo "" > $HOME/ghc/lib/ghc-$ghcversion/ghci-usage.txt
...
# Strip binaries
strip --strip-unneeded $HOME/ghc/lib/ghc-$ghcversion/{run,}ghc
...
# cabal-install
curl --silent http://hackage.haskell.org/packages/archive/cabal-install/$cabalversion/cabal-install-$cabalversion.tar.gz|tar xz
